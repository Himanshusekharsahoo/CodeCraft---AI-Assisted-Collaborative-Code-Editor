  ...
  useEffect(() => {
    const fetchWorkspace = async () => {
      try {
        ...
        toast.error("Failed to fetch workspace. Please try again.");
      }
    };
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-950 text-white min-w-[1024px] relative">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        >
          <PanelLeftOpen size={24} className="h-7 w-7 text-gray-300 hover:text-white transition-colors" />
        </button>
        ...
        >
          {isNavOpen && <NavPanel workspaceId={workspaceId} openFile={setSelectedFile} />}
        </nav>
      ...
      >
        {isChatOpen && <Chat workspaceId={workspaceId} isChatOpen={isChatOpen} setIsChatOpen={setIsChatOpen} />}
      </aside>
