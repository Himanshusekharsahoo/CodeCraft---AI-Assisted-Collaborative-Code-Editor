...
const Dashboard = () => {
  ...
  const handleCreateWorkspace = async () => {
    ...
    try {
      ...
      const workspaceRef = await addDoc(collection(db, "workspaces"), {
        name: workspaceName,
        isPublic,
        description: "A collaborative coding space.",
      });
      ...
      toast.success("Coding space created successfully!", toastOptions);
      setIsOpen(false);
    } catch (error) {
      toast.error("Failed to create coding space.", toastOptions);
    } finally {
      setIsCreating(false);
    }
  };
  ...
  const deleteWorkspace = async (workspaceId) => {
    const confirmationToast = toast(
      <div className="flex justify-between items-center gap-4">
        <span>Are you sure you want to delete this coding space?</span>
          ...
          <Button
            onClick={async () => {
              try {
                ...
                toast.success("Coding space deleted successfully!", toastOptions);
              } catch (error) {
                toast.error("Failed to delete coding space.", toastOptions);
              } finally {
  ...
  return (
    <div className="h-screen w-screen bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950 text-white flex flex-col animate-gradient">
      <ToastContainer />
      <Header />

      <div className="flex justify-between items-center p-6">
        <h1 className="text-4xl border-b border-green-500 font-mono text-green-300">My Coding Spaces</h1>

        <Button
          onClick={createWorkspace}
          className="relative inline-flex items-center px-6 py-3 bg-gradient-to-r from-green-600 to-teal-600 text-white font-semibold rounded-lg shadow-md group hover:from-green-700 hover:to-teal-700 transform transition-all duration-300 ease-out hover:scale-105 z-0"
          disabled={isCreating}
        >
          {isCreating ? (
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {loading ? (
          <p className="text-center text-gray-400">Loading coding spaces...</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {workspaces.length === 0 ? (
              <p className="text-gray-400 col-span-3 text-center">
                No coding spaces found. Create one!
              </p>
            ) : (
              workspaces.map((ws) => (
                <Card
                  key={ws.id}
                  className="relative group border border-green-500 bg-opacity-10 backdrop-blur-md p-2 bg-slate-900 rounded-xl transition-all duration-300 ease-in-out hover:scale-[1.02] hover:shadow-lg hover:shadow-green-500/50"
                >
                  <CardContent className="p-6 flex flex-col gap-4">
                    <Link href={`/workspace/${ws.id}`} className="block">
                      <div className="flex flex-col gap-3">
                        <h2 className="text-3xl font-bold text-green-400 tracking-wide group-hover:text-green-300 transition-colors">
                          {ws.name}
                        </h2>
                        <p className="text-sm text-gray-300 font-mono">
                          {ws.isPublic ? "🌐 Public Space" : "🔒 Private Space"}
                        </p>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        <DialogContent className="bg-[#1E293B] text-white">
          <DialogTitle>New Coding Space</DialogTitle>
          <DialogDescription>
            <p className="mb-2">
              Enter the name of your coding space and choose its visibility.
            </p>
            <Input
              placeholder="Coding Space Name"
              value={workspaceName}
